...
function solution(healths, items) {
    let answer = [];
    let healths_cnt = healths.length;
    let cur_item = [];
    
    const sorted_health = healths.sort((a, b) => (a > b) ? 1 : -1);
    
    for(let i=0; i<healths_cnt; i++) {
        for (let j=0; j<items.length; j++) {
            cur_item = items[j];
            console.log(`${sorted_health[i]} - ${cur_item[1]}`);
            if(parseInt(sorted_health[i]) - parseInt(cur_item[1]) >= 100) {
                console.log(`### ${sorted_health[i]} - ${cur_item[1]}`);
                answer.push(j+1);
                items.splice(j, 1);
                break;
